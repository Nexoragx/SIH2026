  ...
  return (
    <>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2.5">
              ...
              {currentUser ? (
                ...
                <div className="hidden lg:flex items-center gap-2">
                  {/* Accessibility Options Popover for logged out visitors */}
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => setIsAccessibilityOpen(!isAccessibilityOpen)}
              ...
              <button
                ...
                className="flex lg:hidden min-w-[42px] min-h-[42px] items-center justify-center rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-800 border border-slate-200 shadow-2xs transition cursor-pointer p-2"
      ...
      {/* ============================================================
          ...
          Portaled directly to document.body to prevent backdrop-blur
          or sticky header constraints from clipping the drawer!
          ============================================================ */}
      {isMobileMenuOpen &&
        ...
        createPortal(
          <div className="fixed inset-0 z-[9999] lg:hidden">
            ...
            <aside
              className="fixed inset-y-0 right-0 w-[88%] max-w-sm bg-white shadow-2xl z-[10000] flex flex-col justify-between overflow-y-auto border-l border-slate-200 animate-slideInRight"
              role="dialog"
              aria-modal="true"
            ...
            >
              ...
              <div className="p-4 space-y-5 flex-1 overflow-y-auto">
                {/* 1. User Identity or Guest Sign In Card */}
                {currentUser ? (
                  ...
                  <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-200 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black text-indigo-950">Welcome to ANVAYA</span>
                    ...
                    <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                      Confidential psychological monitoring, statutory Rule 12(4) relief tracking, and 24x7 crisis support.
                    </p>
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <button
                        ...
                        className="py-2.5 px-3 rounded-xl bg-white text-indigo-700 font-black text-xs border border-indigo-300 shadow-2xs text-center cursor-pointer min-h-[44px] flex items-center justify-center gap-1.5"
                      >
                        <User className="w-3.5 h-3.5 text-indigo-600" />
                        <span>Sign In</span>
                      </button>
                      <button
                        ...
                        className="py-2.5 px-3 rounded-xl bg-indigo-600 text-white font-black text-xs shadow-xs text-center cursor-pointer min-h-[44px]"
                      >
                        Register
                      </button>
                    </div>
                ...
                {/* 2. SIH Evaluator Persona Fast-Switch (Always accessible for instant testing!) */}
                {onSwitchPersona && (
                  <div className="space-y-2 p-3 bg-slate-50 rounded-2xl border border-slate-200">
                    <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-600">
                      <span className="flex items-center gap-1.5 text-indigo-900">
                        ...
                        <span>SIH Evaluator Persona Switch</span>
                      </span>
                      <span className="text-[9px] text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded font-black">
                        1-Tap Live
                      </span>
                    </div>
                    <p className="text-[10px] text-slate-500 font-medium">
                      Select any role to test its custom dashboard and portal permissions:
                    </p>
                    <div className="grid grid-cols-1 gap-1.5">
                      {[
                        { id: 'citizen', label: '👤 Citizen / Survivor', desc: 'Check-in, DBT relief & exercises' },
                        { id: 'observer', label: '🛡️ District Health Observer', desc: 'Caseload triage (L1–L4)' },
                        { id: 'psychiatrist', label: '🩺 Telepsychiatrist', desc: 'Clinical station & telehealth' },
                        { id: 'ngo', label: '🤝 NGO Field Partner', desc: 'On-ground assistance ops' },
                        { id: 'admin', label: '🏛️ MoSJE Executive Admin', desc: 'National analytics & SLA audits' },
                      ].map((p) => {
                        const isCur = currentUser ? getPersonaKey() === p.id : false;
                        return (
                          <button
                            key={p.id}
                            type="button"
                          ...
                          >
                            <div>
                              <div className={`text-xs font-black ${isCur ? 'text-white' : 'text-slate-900'}`}>
                                {p.label}
                              </div>
                              <div className={`text-[10px] ${isCur ? 'text-indigo-100' : 'text-slate-500'}`}>
                                {p.desc}
                              </div>
                            </div>
                            {isCur && <Check className="w-4 h-4 text-white flex-shrink-0" />}
                          </button>
                        );
                      })}
                    </div>
                ...
                {/* 3. Primary Role Navigation Tabs (when logged in) */}
                {currentUser && (
